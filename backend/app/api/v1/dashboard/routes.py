...
async def get_risk_summary(
    ...
    try:
        ...
        summary = service.get_risk_summary(current_user.current_organization_id)
        return summary
...
async def get_activity_timeline(
    ...
    try:
        service = DashboardService(db)
        timeline = service.get_asset_timeline(current_user.current_organization_id, days)
...
async def get_vulnerable_domains(
    ...
    try:
        service = DashboardService(db)
        domains = service.get_top_vulnerable_domains(current_user.current_organization_id, limit)
...
async def get_scan_statistics(
    ...
    try:
        service = DashboardService(db)
        stats = service.get_scan_statistics(current_user.current_organization_id)
        return stats
    except Exception as e:
...
async def get_risk_heatmap(
    ...
    try:
        service = DashboardService(db)
        heatmap = service.get_domain_risk_heatmap(current_user.current_organization_id)
...
async def get_full_dashboard(
    ...
    try:
        ...
        risk_summary = service.get_risk_summary(current_user.current_organization_id)
        timeline = service.get_asset_timeline(current_user.current_organization_id, 30)
        vulnerable_domains = service.get_top_vulnerable_domains(current_user.current_organization_id, 10)
        scan_stats = service.get_scan_statistics(current_user.current_organization_id)
        heatmap = service.get_domain_risk_heatmap(current_user.current_organization_id)
