...
from app.dependencies import get_current_user, require_tenant_member, require_org_admin, require_super_admin
import logging
...
def _owned_vulnerability_query(db: Session, user_id: str):
    ...
    return (
        ...
        .filter(Asset.organization_id == user_id)
    )
...
async def list_vulnerabilities(
    ...
    try:
        base_query = _owned_vulnerability_query(db, current_user.current_organization_id)
        ...
        scan_rows = (
            ...
            .join(Asset, Scan.asset_id == Asset.id)
            .filter(Asset.organization_id == current_user.current_organization_id)
            .group_by(
                Scan.id,
                Scan.reference_id,
                Scan.target_domain,
