...
def _owned_domain(db: Session, user_id: str, domain_id: str) -> Domain:
    domain = (
        ...
        .filter(Domain.id == domain_id, Asset.user_id == user_id)
        .first()
    )
    if not domain:
...
async def recon_assets(
    ...
    assets = (
        db.query(Asset)
        .filter(Asset.user_id == current_user.id, Asset.status == "active")
        .order_by(Asset.name.asc())
        .all()
    )
    items = []
    for asset in assets:
        if asset.asset_type not in {"domain", "subdomain", "url", "web_application"}:
            continue
...
async def start_recon(
    ...
    asset = (
        db.query(Asset)
        .filter(Asset.id == request.asset_id, Asset.user_id == current_user.id, Asset.status == "active")
        .first()
    )
    if not asset:
    ...
    if request.domain_id:
        domain = _owned_domain(db, current_user.id, request.domain_id)
