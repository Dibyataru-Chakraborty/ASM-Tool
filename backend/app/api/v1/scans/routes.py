...
async def list_scans(
    ...
    try:
        ...
        query = db.query(Scan).join(Asset).filter(Asset.organization_id == current_user.current_organization_id)
        if asset_id:
...
async def get_scan_archive(
    ...
    payload = archive_service.read_archive(current_user.current_organization_id, scan.id)
    if payload is None:
...
async def delete_scan(
    ...
    try:
        ...
        owner_id = current_user.current_organization_id
