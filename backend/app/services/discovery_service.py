...
from app.models import Asset, DNSRecord, Domain, Scan, Subdomain
...
class DiscoveryService:
    ...
    def run_real_scan(self, scan_id: str, domain_id: str):
        ...
        from app.utils.database import configure_session_rls
        ...
        try:
            ...
            configure_session_rls(self.db, bypass=True)
            ...
            self.db.commit()
            # Convert the completed point-in-time scan into persistent ASM state.
            # This is deliberately non-fatal: a successful factual scan remains
            # successful even if inventory synchronization needs repair.
            try:
                from app.services.attack_surface_service import AttackSurfaceService

                AttackSurfaceService(self.db).sync_from_scan(scan_id, domain_id)
            except Exception:
                self.db.rollback()
                logger.exception(
                    "Could not synchronize completed scan %s into the ASM inventory",
                    scan_id,
                )
            try:
