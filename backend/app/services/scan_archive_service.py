...
class ScanArchiveService:
    ...
    def archive_scan(
        ...
        *,
        ...
        destination = self.archive_path(asset.organization_id, scan.id)
        if destination.is_file() and not overwrite:
            return destination
    ...
    def backfill_missing(self, db: Session) -> int:
        ...
        for scan_id in scan_ids:
            scan_asset = (
                db.query(Scan.id, Asset.organization_id)
                .join(Asset, Scan.asset_id == Asset.id)
                .filter(Scan.id == scan_id)
                .first()
            )
            if not scan_asset:
                continue
            if self.archive_path(scan_asset.organization_id, scan_id).is_file():
                continue
            try:
