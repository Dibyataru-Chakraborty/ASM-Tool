...
def _enable_worker_access(db) -> None:
    """Grant the trusted internal scheduler transaction-local RLS bypass."""
...
def _claim_due_schedule(schedule_id: str) -> tuple[str, str] | None:
    ...
    try:
        ...
        if not asset:
            raise RuntimeError("Scheduled asset is missing, archived, or outside the schedule organization")
