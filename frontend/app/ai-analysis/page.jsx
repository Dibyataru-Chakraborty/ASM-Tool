...
const PROVIDERS = [
  {
    ...
    name: "Claude",
    icon: "🟣",
    desc: "Anthropic — Best for detailed analysis",
  },
  {
    id: "openai",
    name: "GPT-4",
    icon: "🟢",
    desc: "OpenAI — Fast and reliable",
  },
  {
    id: "gemini",
    name: "Gemini Pro",
    icon: "🔵",
    desc: "Google — Excellent reasoning",
  },
  {
    id: "all",
    name: "All Providers",
    icon: "⚡",
    desc: "Compare results from all AI providers",
  },
];
...
const TASKS = [
  {
    ...
    desc: "Deep analysis of a specific CVE",
  },
  {
    id: "prioritize",
    icon: List,
    label: "Prioritize Vulns",
  ...
  },
  {
    ...
    desc: "Business-ready risk summary",
  },
  {
    id: "remediate",
    icon: Zap,
    label: "Remediation Steps",
    desc: "Step-by-step fix instructions",
  },
];
    ...
    try {
      ...
      setError(
        e.response?.data?.detail ||
          "AI analysis failed. Ensure your API key is configured in backend/.env",
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-gray-500 mt-0.5">
          Use Claude, OpenAI, or Gemini to analyze vulnerabilities and generate
          insights
        </p>
      </div>
      ...
      {providers && (
        <div className="card p-4">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {Object.entries(providers.providers).map(([id, p]) => (
              <div
                key={id}
              ...
              >
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span
                    className={p.available ? "text-gray-200" : "text-gray-500"}
                  >
                    {p.name}
                  </span>
                </div>
                <p className="text-gray-500 text-[10px]">
                  {p.available ? "API key configured" : "No API key"}
                </p>
              </div>
            ))}
          </div>
          ...
          {!result && !error && !loading && (
            <div className="flex flex-col items-center justify-center py-20 text-center gap-3">
              ...
              <p className="text-sm text-gray-500">
                Configure and run an analysis to see AI insights here
              </p>
            </div>
          )}
              ...
              {result.remediation_steps && (
                <ol className="space-y-2">
                  {result.remediation_steps.map((step, i) => (
                    <li
                      key={i}
                      className="flex gap-3 text-xs text-gray-300 p-3 rounded-lg bg-[#0d1117] border border-[#30363d]"
                    >
              ...
              {typeof result === "object" &&
                ...
                !result.remediation_steps && (
                  <div className="space-y-3">
                    {Object.entries(result).map(([provider, data]) => (
                      <div
                        key={provider}
                        className="p-3 rounded-lg border border-[#30363d] bg-[#0d1117]"
                      >
                        <p className="text-xs font-medium text-blue-400 mb-2 capitalize">
                          {provider}
                        </p>
                        {data.error ? (
