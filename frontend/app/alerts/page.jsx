...
function AlertsContent() {
  ...
  const critical = displayAlerts.filter(
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-xs text-gray-500 mt-0.5">
          {open} open alerts requiring attention
        </p>
      </div>
      ...
      <div className="card">
        {displayAlerts.length === 0 ? (
          <EmptyState
            title="No alerts"
            description="All clear! No active alerts at this time."
          />
        ) : (
          <Table
            headers={[
              ...
              <tr key={a.id} className="table-row">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    <AlertTriangle
                      className={`w-3.5 h-3.5 shrink-0 ${a.is_resolved ? "text-gray-500" : "text-orange-400"}`}
                    />
                ...
                <td className="px-4 py-3 max-w-xs">
                  <p
                    className={`text-xs line-clamp-2 ${a.is_resolved ? "text-gray-500 line-through" : "text-gray-300"}`}
                  >
                    {a.message}
                  </p>
                </td>
                <td className="px-4 py-3">
                  <span
                    className={`text-xs font-medium ${a.is_resolved ? "text-green-400" : "text-orange-400"}`}
                  >
