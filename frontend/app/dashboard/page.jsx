...
import { SuperAdminContent } from "../super-admin/page";
  ...
  const cards = [
    {
      ...
      hint: user?.organization_name || "Current organization",
      icon: Boxes,
      href: "/attack-surface",
    },
    {
      label: "New Assets",
      value: overview?.new_assets || 0,
      hint: "Discovered since prior observation",
      icon: Sparkles,
      href: "/attack-surface?status=new",
    },
    {
      label: "Unknown / Review",
      value: overview?.unknown_assets || 0,
      hint: "Ownership requires investigation",
      icon: Eye,
      href: "/attack-surface?ownership_status=requires_investigation",
    },
    {
      label: "Exposed Assets",
      value: overview?.exposed_assets || 0,
      hint: `${overview?.critical_exposures || 0} critical exposures`,
      icon: ShieldAlert,
      href: "/exposures",
    },
    {
      label: "Changes (24h)",
      value: overview?.changes_24h || 0,
      hint: "New, changed, removed or resolved",
      icon: Activity,
      href: "/changes",
    },
    {
      label: "30d Surface Growth",
      value: `${overview?.attack_surface_growth_30d || 0}%`,
      hint: "Based on first-seen inventory",
      icon: TrendingUp,
      href: "/attack-surface",
    },
  ];
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-gray-500">
              Persistent external asset inventory, exposure context and change
              monitoring.
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-3 xl:grid-cols-6">
          {cards.map(({ label, value, hint, icon: Icon, href }) => (
            <Link
              key={label}
              href={href}
              className="card p-4 transition hover:border-blue-500/40"
            >
          ...
          <div className="card p-4">
            <div className="mb-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-600">
                  Current active attack surface by type
                </p>
              </div>
            ...
            {Object.keys(inventoryByType).length === 0 ? (
              <div className="py-10 text-center text-xs text-gray-600">
                Run discovery to build the persistent inventory.
              </div>
            ) : (
              <div className="space-y-3">
                {Object.entries(inventoryByType)
                  ...
                  .map(([type, count]) => {
                    const max = Math.max(
                      ...Object.values(inventoryByType).map((v) => Number(v)),
                      1,
                    );
                    return (
                      <div key={type}>
                        ...
                        <div className="h-1.5 overflow-hidden rounded-full bg-[#21262d]">
                          <div
                            ...
                            style={{
                              width: `${Math.max(4, (Number(count) / max) * 100)}%`,
                            }}
                          />
                        </div>
                      </div>
          ...
          <div className="card p-4">
            <div className="mb-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-600">
                  Open exposures, prioritized beyond CVSS alone
                </p>
              </div>
            ...
            <div className="grid grid-cols-5 gap-2">
              {["critical", "high", "medium", "low", "info"].map((sev) => (
                <Link
                  href={`/exposures?severity=${sev}`}
                  key={sev}
                  className="rounded-lg border border-[#30363d] p-3 text-center hover:border-blue-500/40"
                >
                  <p className={`text-xl font-bold ${SEV_CLS[sev]}`}>
                    {exposureBySeverity[sev] || 0}
                  </p>
            ...
            <div className="flex items-center justify-between border-b border-[#21262d] px-4 py-3">
              <div>
                ...
                <p className="text-[10px] text-gray-600">
                  What changed between monitoring cycles
                </p>
              </div>
              <Link href="/changes" className="text-xs text-blue-400">
                View all
              </Link>
            </div>
            {recentChanges.length === 0 ? (
              <div className="py-10 text-center text-xs text-gray-600">
                No attack-surface changes recorded yet.
              </div>
            ) : (
              <div className="divide-y divide-[#21262d]">
                {recentChanges.map((change) => (
                  ...
                  >
                    <span
                      className={`mt-1 h-2 w-2 shrink-0 rounded-full ${change.severity === "critical" ? "bg-red-400" : change.severity === "high" ? "bg-orange-400" : change.severity === "medium" ? "bg-yellow-400" : "bg-blue-400"}`}
                    />
          ...
          <div className="card overflow-hidden">
            <div className="flex items-center justify-between border-b border-[#21262d] px-4 py-3">
              <div>
                ...
                <p className="text-[10px] text-gray-600">
                  Internet exposure + severity + business criticality
                </p>
              </div>
              <Link href="/attack-surface" className="text-xs text-blue-400">
                Inventory
              </Link>
            </div>
            {topRisk.length === 0 ? (
              <div className="py-10 text-center text-xs text-gray-600">
                No prioritized assets yet.
              </div>
            ) : (
              <div className="divide-y divide-[#21262d]">
                {topRisk.map((asset) => (
                  <Link
                    ...
                    key={asset.id}
                    className="flex items-center gap-3 px-4 py-3 hover:bg-[#1c2128]"
                  >
                    <div className="min-w-0 flex-1">
                    ...
                    <div className="text-right">
                      <p
                        className={`${asset.risk_score >= 80 ? "text-red-400" : asset.risk_score >= 60 ? "text-orange-400" : asset.risk_score >= 40 ? "text-yellow-400" : "text-blue-400"} text-sm font-bold`}
                      >
                        {asset.risk_score}
                      </p>
