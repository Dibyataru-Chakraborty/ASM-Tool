...
function ReportDetailPageInner({ params }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    asm
      ...
      .then((r) => {
        ...
        if (r.scan_job_id) {
          asm
            .getVulns({ scan_job_id: r.scan_job_id, limit: 200 })
            .then((v) => setVulns(v.vulnerabilities || []));
        }
      })
      .catch(() => {});
  }, [id]);
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-3 flex-wrap">
              <span className={`text-2xl font-bold ${riskCls}`}>
                {report.risk_score?.toFixed(1)}/10
              </span>
              <span className={`text-sm font-semibold ${riskCls}`}>
            ...
            <div className="grid grid-cols-5 gap-3 mb-4">
              {[
                ...
                <div
                  key={s.l}
                  className={`rounded-xl border p-3 text-center ${s.c}`}
                >
                  <p className={`text-xl font-bold ${s.c.split(" ")[0]}`}>
                    {s.v}
                  </p>
          ...
          <div className="flex flex-col gap-2 shrink-0">
            <a
              href={asm.exportReport(id, "markdown")}
              download
              className="btn-blue text-xs text-center"
            >
              ⬇ Download .md
            </a>
            <a
              href={asm.exportReport(id, "json")}
              download
              className="btn-gray text-xs text-center"
            >
              ⬇ Download .json
            </a>
            <Link
              href={`/scans/${report.scan_job_id}`}
              className="btn-gray text-xs text-center"
            >
              🔭 View Scan
            </Link>
          ...
          {report.attack_surface && (
            <div className="card p-5">
              ...
              <div className="grid grid-cols-3 gap-4">
                {Object.entries(report.attack_surface).map(([k, v]) => (
                  <div
                    key={k}
          ...
          {report.subdomains_found?.length > 0 && (
            <div className="card p-5">
              ...
              <div className="grid grid-cols-2 gap-1 max-h-64 overflow-y-auto">
                {report.subdomains_found.map((s) => (
                  <p
                    key={s}
                    className="text-xs font-mono text-gray-300 bg-[#0d1117] border border-[#21262d] px-2 py-1 rounded"
                  >
                    {s}
                  </p>
                ))}
              ...
              <div className="flex flex-wrap gap-1.5">
                {report.open_ports
                  ...
                  .map((p) => (
                    <span
                      key={p}
                      className="text-xs font-mono bg-[#0d1117] border border-[#30363d] text-gray-300 px-2 py-0.5 rounded"
                    >
                      {p}
                    </span>
                  ))}
        ...
        <div className="card overflow-hidden">
          {vulns.length === 0 ? (
            <div className="py-10 text-center text-gray-600 text-sm">
              No vulnerabilities in this report
            </div>
          ) : (
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-[#21262d]">
                  {["Title", "Severity", "CVSS", "CVE", "Host"].map((h) => (
                    <th
                      key={h}
                      className="text-left px-4 py-3 text-gray-500 font-medium uppercase tracking-wide"
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tbody>
                {vulns.map((v) => (
                  <tr
                    key={v.id}
                    className="border-b border-[#21262d] hover:bg-[#1c2128] transition"
                  >
                    <td className="px-4 py-2.5 max-w-xs">
                      <Link
                        href={`/vulnerabilities/${v.id}`}
                        className="text-gray-200 hover:text-blue-400 truncate block"
                      >
                        {v.title}
                      </Link>
                    </td>
                    <td className="px-4 py-2.5">
                      <span className={SEV_CLS[v.severity] || "tag-info"}>
                        {v.severity}
                      </span>
                    </td>
                    <td className="px-4 py-2.5">
                      <span
                        className={`font-bold ${(v.cvss_score || 0) >= 9 ? "text-red-400" : (v.cvss_score || 0) >= 7 ? "text-orange-400" : (v.cvss_score || 0) >= 4 ? "text-yellow-400" : "text-blue-400"}`}
                      >
                        {v.cvss_score?.toFixed(1) || "—"}
                      </span>
                    </td>
                    <td className="px-4 py-2.5 font-mono text-blue-400 text-[10px]">
                      {v.cve_id || "—"}
                    </td>
                    <td className="px-4 py-2.5 font-mono text-gray-400 truncate max-w-[120px]">
                      {v.host || "—"}
                    </td>
                  </tr>
                ))}
