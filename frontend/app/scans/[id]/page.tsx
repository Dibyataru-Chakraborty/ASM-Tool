...
type AIServiceAssessment = {
  ...
  provider: string
  model_name: string
  lifecycle_status: 'current' | 'outdated' | 'unknown' | string
  severity?: string | null
  title: string
  summary: string
...
type ReconResults = {
  ...
  ips: Array<{ ip: string; subdomains: string[]; subdomain_count: number }>
  vulnerabilities: ReconVulnerability[]
  aiAssessments: AIServiceAssessment[]
  screenshots: ReconScreenshot[]
}

const ACTIVE_STATUSES = new Set(['pending', 'queued', 'running'])

const SEVERITY_META: Record<string, { label: string; color: string; className: string; fallback: number }> = {
  critical: { label: 'Critical', color: '#ef4444', className: 'text-red-400 bg-red-500/10 border-red-500/20', fallback: 95 },
  high: { label: 'High', color: '#f97316', className: 'text-orange-400 bg-orange-500/10 border-orange-500/20', fallback: 80 },
  medium: { label: 'Medium', color: '#eab308', className: 'text-yellow-400 bg-yellow-500/10 border-yellow-500/20', fallback: 55 },
  low: { label: 'Low', color: '#3b82f6', className: 'text-blue-400 bg-blue-500/10 border-blue-500/20', fallback: 25 },
  info: { label: 'Info', color: '#64748b', className: 'text-gray-400 bg-gray-500/10 border-gray-500/20', fallback: 0 },
}

function severityKey(value?: string | null) {
...
function RiskMeter({ score }: { score: number }) {
  ...
  const label = score >= 90 ? 'Critical' : score >= 70 ? 'High' : score >= 40 ? 'Medium' : score > 0 ? 'Low' : 'Clean'
  return (
    <div className="flex items-center gap-5">
      <div
        className="relative grid h-28 w-28 shrink-0 place-items-center rounded-full"
        style={{ background: `conic-gradient(${color} ${score * 3.6}deg, var(--asm-surface-raised) 0deg)` }}
      >
        <div className="grid h-20 w-20 place-items-center rounded-full bg-[#161b22] text-center">
          <div>
            ...
            <p className="text-[9px] uppercase tracking-wide text-gray-500">of 100</p>
          </div>
        </div>
      </div>
      <div>
        <p className="text-xs uppercase tracking-wide text-gray-500">Vulnerability risk</p>
        <p className="mt-1 text-lg font-bold" style={{ color }}>{label}</p>
        <p className="mt-1 text-xs text-gray-500">Calculated from persisted scanner evidence and grounded AI service assessments.</p>
      </div>
    </div>
  )
}

function SeverityColumns({ findings }: { findings: Array<{ severity?: string | null }> }) {
  const order = ['critical', 'high', 'medium', 'low', 'info']
  const counts = Object.fromEntries(order.map(key => [key, 0])) as Record<string, number>
  ...
  return (
    <div className="grid h-36 grid-cols-5 items-end gap-3">
      {order.map(key => {
        ...
        const height = count ? Math.max(14, Math.round((count / maximum) * 88)) : 4
        return (
          <div key={key} className="flex h-full flex-col items-center justify-end gap-2">
            <span className="text-sm font-bold text-gray-200">{count}</span>
            <div
              className="w-full max-w-10 rounded-t-md transition-all"
              style={{ height, backgroundColor: meta.color, opacity: count ? 1 : 0.2 }}
            />
            <span className="text-[9px] uppercase text-gray-500">{meta.label}</span>
          </div>
        )
      })}
    </div>
...
function ResultsEmpty({ message }: { message: string }) {
  return (
...
function ScanDetailPageInner({ params }: { params: { id: string } }) {
  ...
  const loadResults = useCallback(async () => {
    ...
    try {
      ...
      const detail = requestError?.response?.data?.detail
      setResults(null)
      setResultsError(typeof detail === 'string' ? detail : 'Unable to load persisted scanner results.')
    } finally {
  ...
  const serviceRows = (results?.subdomains || []).flatMap(subdomain => {
    const servicesByPort = new Map(subdomain.services.map(service => [service.port, service]))
  ...
  const nucleiRisk = results?.vulnerabilities.reduce((highest, vulnerability) => {
    const cvssRisk = typeof vulnerability.cvss_score === 'number'
  ...
  const aiRisk = results?.aiAssessments.reduce((highest, assessment) => (
    Math.max(highest, SEVERITY_META[severityKey(assessment.severity)].fallback)
  ), 0) || 0
        ...
        <div className={`flex items-start gap-3 rounded-xl border p-4 ${
          ...
          <div>
            <p className={`text-xs font-semibold ${
              ...
              {status === 'completed' ? 'Scan completed with warning' : 'Scan failed'}
            </p>
            <p className={`mt-1 whitespace-pre-wrap text-xs ${
              status === 'completed' ? 'text-yellow-100/80' : 'text-red-200/80'
            }`}>{scan.error_message}</p>
          </div>
        </div>
      )}
          ...
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-base font-semibold text-gray-100">Real scanner findings</h2>
              </div>
              <p className="mt-1 text-xs text-gray-500">
                Persisted output from the real multi-stage reconnaissance pipeline.
              </p>
            </div>
            <button
              type="button"
            ...
            >
              <Loader2 className={`h-3.5 w-3.5 ${resultsLoading ? 'animate-spin' : ''}`} />
              Refresh findings
            </button>
          </div>
              ...
              <div className="card overflow-hidden">
                <div className="flex items-center justify-between border-b border-[#21262d] px-5 py-4">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-gray-500">Asset discovery enriched with DNS and web-response evidence.</p>
                  </div>
                  <span className="text-xs font-semibold text-cyan-400">{results.subdomains.length}</span>
                </div>
                {results.subdomains.length === 0 ? (
                  <div className="p-4"><ResultsEmpty message="No persisted subdomains were found." /></div>
                ) : (
                  <div className="overflow-x-auto">
                    <table className="w-full min-w-[760px] text-xs">
                      <thead>
                        <tr className="border-b border-[#21262d]">
                          {['Host', 'Resolved IPs', 'HTTP', 'Technologies', 'Open ports'].map(header => (
                      ...
                      <tbody>
                        {results.subdomains.map(subdomain => (
                          <tr key={subdomain.id} className="border-b border-[#21262d] transition hover:bg-[#1c2128]">
                            <td className="px-4 py-3 font-mono text-gray-200">{subdomain.subdomain}</td>
                            <td className="px-4 py-3">
                              <div className="flex flex-wrap gap-1">
                                {subdomain.ip_addresses.length
                                  ? subdomain.ip_addresses.map(ip => (
                                      <code key={ip} className="rounded bg-blue-500/10 px-1.5 py-0.5 text-[10px] text-blue-400">{ip}</code>
                                    ))
                            ...
                            <td className="px-4 py-3">
                              {subdomain.is_responsive
                                ? <span className="text-green-400">{subdomain.response_status_code || 'Responsive'}{subdomain.has_ssl ? ' · TLS' : ''}</span>
                                : <span className="text-gray-600">Not responsive</span>}
                            </td>
                            <td className="px-4 py-3">
                              <div className="flex max-w-xs flex-wrap gap-1">
                                {subdomain.technologies.length
                                  ? subdomain.technologies.map(technology => (
                                      <span key={technology} className="rounded border border-[#30363d] bg-[#0d1117] px-1.5 py-0.5 text-[10px] text-gray-400">{technology}</span>
                                    ))
                                  : <span className="text-gray-600">—</span>}
                              </div>
                            </td>
                            <td className="px-4 py-3 font-mono text-purple-400">
                              {subdomain.open_ports.length ? subdomain.open_ports.join(', ') : '—'}
                            </td>
                          </tr>
                        ))}
                      </tbody>
              ...
              <div className="card overflow-hidden">
                <div className="flex items-center justify-between border-b border-[#21262d] px-5 py-4">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-gray-500">Open ports with factual product and version detection.</p>
                  </div>
                  <span className="text-xs font-semibold text-purple-400">{serviceRows.length}</span>
                </div>
                {serviceRows.length === 0 ? (
                  <div className="p-4"><ResultsEmpty message="No open services were persisted." /></div>
                ) : (
                  <div className="max-h-96 overflow-auto">
                    <table className="w-full min-w-[700px] text-xs">
                      <thead className="sticky top-0 bg-[#161b22]">
                        <tr className="border-b border-[#21262d]">
                          {['Host', 'Port', 'Protocol', 'Service', 'Product / version'].map(header => (
                            <th key={header} className="px-4 py-3 text-left font-medium uppercase tracking-wide text-gray-500">{header}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {serviceRows.map((service, index) => (
                          <tr key={`${service.host}-${service.port}-${index}`} className="border-b border-[#21262d] transition hover:bg-[#1c2128]">
                            <td className="px-4 py-3 font-mono text-gray-300">{service.host}</td>
                            <td className="px-4 py-3 font-mono font-semibold text-purple-400">{service.port}</td>
                            <td className="px-4 py-3 text-gray-500">{service.protocol}</td>
              ...
              <div className="card overflow-hidden">
                <div className="flex items-center justify-between border-b border-[#21262d] px-5 py-4">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-gray-500">Grounded version and CVE checks are kept separate from scanner-confirmed findings.</p>
                  </div>
                  <span className="text-xs font-semibold text-pink-400">{results.aiAssessments.length}</span>
                </div>
                {results.aiAssessments.length === 0 ? (
                  <div className="p-4"><ResultsEmpty message="No AI service assessments were persisted. Configure the AI API key and run a new Full Recon scan." /></div>
                ) : (
                  <div className="max-h-[32rem] overflow-auto">
                    <table className="w-full min-w-[980px] text-xs">
                      <thead className="sticky top-0 bg-[#161b22]">
                        <tr className="border-b border-[#21262d]">
                          {['Severity', 'Affected service', 'Version status', 'Assessment', 'Evidence'].map(header => (
                            <th key={header} className="px-4 py-3 text-left font-medium uppercase tracking-wide text-gray-500">{header}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {results.aiAssessments.map(assessment => {
                          const lifecycleClass = assessment.lifecycle_status === 'current'
                            ? 'text-green-400'
                            : assessment.lifecycle_status === 'outdated'
                              ? 'text-orange-400'
                              : 'text-gray-500'
                          return (
                            <tr key={assessment.id} className="border-b border-[#21262d] align-top transition hover:bg-[#1c2128]">
                              <td className="px-4 py-3"><SeverityBadge severity={assessment.severity} /></td>
                              <td className="px-4 py-3">
                                <p className="font-mono text-gray-300">{assessment.host}:{assessment.port}</p>
                                <p className="mt-1 text-[10px] text-gray-500">{assessment.product || assessment.service_name}</p>
                              </td>
                              <td className="px-4 py-3">
                                <p className={`font-semibold capitalize ${lifecycleClass}`}>{assessment.lifecycle_status}</p>
                                <p className="mt-1 font-mono text-[10px] text-gray-500">
                                  Detected: {assessment.detected_version || 'unknown'}
                                </p>
                                {assessment.latest_version && (
                                  <p className="mt-0.5 font-mono text-[10px] text-green-500">Latest: {assessment.latest_version}</p>
                                )}
                              </td>
                              <td className="max-w-xl px-4 py-3">
                                <p className="font-medium text-gray-200">{assessment.title}</p>
                                <p className="mt-1 text-[10px] leading-relaxed text-gray-500">{assessment.summary}</p>
                                ...
                                {assessment.cves.length > 0 && (
                                  <div className="mt-2 flex flex-wrap gap-1">
                                    {assessment.cves.map(cve => (
                                      <span key={cve.cve_id} className="rounded border border-red-500/20 bg-red-500/10 px-1.5 py-0.5 font-mono text-[10px] text-red-300">
                                        {cve.cve_id}{typeof cve.cvss_score === 'number' ? ` · ${cve.cvss_score.toFixed(1)}` : ''}
                                      </span>
                                    ))}
                                  </div>
                                )}
                              ...
                              <td className="px-4 py-3">
                                <p className="mb-2 text-[10px] text-gray-500">Confidence {Math.round((assessment.confidence || 0) * 100)}%</p>
                                <div className="flex max-w-xs flex-col gap-1">
                                  {assessment.evidence_urls.length
                                    ? assessment.evidence_urls.slice(0, 3).map((url, index) => (
                                        <a
                                          key={url}
                                          href={url}
                                          target="_blank"
                                    ...
                                    : <span className="text-[10px] text-gray-600">No cited evidence</span>}
                                </div>
                              </td>
                            </tr>
                          )
                ...
                {results.vulnerabilities.length === 0 ? (
                  ...
                  <div className="max-h-[32rem] overflow-auto">
                    <table className="w-full min-w-[820px] text-xs">
                      <thead className="sticky top-0 bg-[#161b22]">
                        <tr className="border-b border-[#21262d]">
                          {['Severity', 'Finding', 'CVE', 'Affected service', 'CVSS'].map(header => (
                            <th key={header} className="px-4 py-3 text-left font-medium uppercase tracking-wide text-gray-500">{header}</th>
                          ))}
                        </tr>
                      </thead>
                        ...
                        {results.vulnerabilities.map(vulnerability => (
                          <tr key={vulnerability.id} className="border-b border-[#21262d] align-top transition hover:bg-[#1c2128]">
                            ...
                            <td className="max-w-lg px-4 py-3">
                              <p className="font-medium text-gray-200">{vulnerability.title}</p>
                              {vulnerability.description && (
                                <p className="mt-1 line-clamp-2 text-[10px] leading-relaxed text-gray-500">{vulnerability.description}</p>
                              )}
                            </td>
              ...
              <div className="card overflow-hidden">
                <div className="flex items-center justify-between border-b border-[#21262d] px-5 py-4">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-gray-500">Captured only from responsive web URLs.</p>
                  </div>
                  <span className="text-xs font-semibold text-green-400">{results.screenshots.length}</span>
                </div>
                {results.screenshots.length === 0 ? (
                  <div className="p-4"><ResultsEmpty message="No valid screenshots were persisted for this scan." /></div>
                ) : (
                  <div className="grid gap-3 p-4 sm:grid-cols-2 xl:grid-cols-3">
                    {results.screenshots.map(screenshot => (
                      <article key={screenshot.id} className="overflow-hidden rounded-lg border border-[#30363d] bg-[#0d1117]">
                        {screenshot.file_url ? (
                          <img
                        ...
                        <div className="p-3">
                          <p className="truncate font-mono text-xs text-gray-200">{screenshot.subdomain}</p>
                          <p className="mt-1 truncate text-[10px] text-gray-500">{screenshot.title || screenshot.url}</p>
                          <p className="mt-2 text-[10px] text-green-400">{screenshot.status_code || 'HTTP response captured'}</p>
                        </div>
                      </article>
                    ))}
                  </div>
