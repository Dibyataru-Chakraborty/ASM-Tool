...
export default function ScansPage() {
  ...
  const load = async () => {
    ...
    const scanRows = (s.items || s.scans || []).map((scan) => {
      ...
      return {
        ...
        status,
        asset_target:
          scan.asset_target || asset?.target || asset?.name || scan.asset_id,
    ...
    const statusRows = filter
      ? scanRows.filter((scan) => scan.status === filter)
      : scanRows;
  ...
  }, [filter, assetFilter, searchQuery]);

  const cancel = async (id) => {
    await asm.cancelScan(id);
    load();
  };
      ...
      <AppLayout>
        <div className="space-y-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500">
                {scans.length} scans · Auto-refreshes every 5s
              </p>
            </div>
          ...
          <div className="card overflow-x-auto">
            {loading ? (
              <div className="py-12 text-center animate-pulse text-gray-600 text-sm">
                Loading scans…
              </div>
            ) : scans.length === 0 ? (
              <div className="py-12 text-center">
                ...
                <Link
                  href="/assets"
                  className="btn-blue text-xs mt-3 inline-block"
                >
                  Go to Assets → Trigger Scan
                </Link>
              </div>
            ) : (
                ...
                <tbody>
                  {scans.map((j) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5">
                          {j.status === "running" ? (
                            ...
                            <span
                              className={`h-2 w-2 rounded-full ${STATUS_DOT[j.status] || "bg-gray-600"}`}
                            />
                          )}
                          <span
                            className={STATUS_CLS[j.status] || "text-gray-400"}
                          >
                            {j.status}
                          </span>
                        </div>
                        ...
                        <div className="h-1.5 bg-[#21262d] rounded-full overflow-hidden">
                          {j.status === "running" ? (
                            <div
                              ...
                              style={{
                                width: `${Math.max(4, Math.min(100, j.progress || 0))}%`,
                              }}
                            />
                          ) : (
                            <div
                      ...
                      <td className="px-4 py-3 text-gray-500">
                        {j.duration_seconds
                          ? `${Math.round(j.duration_seconds / 60)}m ${j.duration_seconds % 60}s`
                          : "—"}
                      </td>
                      <td className="px-4 py-3">
              ...
              <div className="flex items-start gap-3 border-b border-[#30363d] px-5 py-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-xs leading-5 text-gray-500">
                    This action permanently deletes this scan and its associated
                    scan history.
                  </p>
                </div>
                ...
                {["running", "queued"].includes(deleteTarget.status) && (
                  ...
                  >
                    <p className="text-xs leading-5 text-orange-300">
                      This scan is currently {deleteTarget.status}. Cancel the
                      scan first before deleting it.
                    </p>
                  </div>
                )}
