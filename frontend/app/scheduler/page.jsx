...
function SchedulerContent() {
  ...
  const load = async () => {
    setLoading(true);
    setError("");
    const [scheduleResult, assetResult, mailResult, userResult] =
    ...
    if (scheduleResult.status === "fulfilled") {
      ...
      setError(
        scheduleResult.reason?.response?.data?.detail ||
    ...
    if (assetResult.status === "fulfilled") {
      const availableAssets =
        assetResult.value.items || assetResult.value.assets || [];
      ...
      setError(
        (current) =>
          current ||
          assetResult.reason?.response?.data?.detail ||
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(async () => {
      try {
        ...
        setPreview(null);
        setPreviewError(
          previewFailure.response?.data?.detail ||
  ...
  const sendTestEmail = async () => {
    ...
    setMailMessage("");
    try {
      const result = await asm.sendScheduleTestEmail(
        form.notify_email || undefined,
      );
      ...
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500">
            Persistent timezone-aware discovery and exposure monitoring
          </p>
        </div>
        <div className="flex gap-2">
          <button
            type="button"
            onClick={() => void load()}
            className="btn-gray inline-flex items-center gap-2 text-sm"
          >
          ...
          <button
            ...
            className="btn-blue inline-flex items-center gap-2 text-sm"
          >
          ...
          <form onSubmit={save} className="space-y-4">
            ...
            {form.notify_on_completion && (
              <div className="rounded-lg border border-[#30363d] bg-[#0d1117] p-3">
                <div className="mb-2 flex items-center justify-between gap-3">
                  ...
                  <span
                    className={`text-[10px] ${mailStatus?.configured ? "text-green-400" : "text-yellow-400"}`}
                  >
                ...
                {!mailStatus?.configured && (
                  <p className="mt-2 text-[10px] text-yellow-400">
                    Add {mailStatus?.missing?.join(", ") || "SMTP settings"} to
                    backend/.env.
                  </p>
                )}
                {mailMessage && (
            ...
            <div className="flex gap-2">
              <button
                ...
                className="btn-gray flex-1 text-sm"
              >
                Cancel
              </button>
              <button
                ...
                {schedules.map((schedule) => (
                  ...
                  >
                    <td className="px-4 py-3">
                      ...
                      <p className="max-w-52 truncate font-mono text-[10px] text-gray-500">
                        {schedule.target}
                      </p>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      {schedule.notify_on_completion ? (
                        <>
                          <p className="max-w-40 truncate text-gray-400">
                          ...
                          <p
                            className={`mt-1 text-[10px] ${
                              schedule.notification_status === "sent"
                                ? "text-green-400"
                                : schedule.notification_status?.startsWith(
                                      "failed",
                                    )
                                  ? "text-red-400"
                                  : "text-gray-600"
