...
function SettingsContent() {
  ...
  useEffect(() => {
    client
      .get("/api/v1/recon/providers/status")
  ...
  const changePw = async (e) => {
    ...
    try {
      ...
      setPwMsg("❌ " + (e.response?.data?.detail || "Failed"));
    }
        ...
        <form onSubmit={changePw} className="space-y-3">
          <div>
            ...
            <input
              ...
              value={pwForm.old}
              onChange={(e) =>
                setPwForm((f) => ({ ...f, old: e.target.value }))
              }
            />
          </div>
          <div>
            ...
            <input
              ...
              value={pwForm.new_pw}
              onChange={(e) =>
                setPwForm((f) => ({ ...f, new_pw: e.target.value }))
              }
            />
          </div>
          <div>
      ...
      {providers && (
        <>
          {SECTIONS.map((section) => (
            <div key={section.title} className="card p-5">
                ...
                {section.keys.map((k) => {
                  ...
                  const configured =
                    allProviders[
                      k.key
                        .toLowerCase()
                        .replace(/_api_key|_api_id|_api_secret/g, "")
                    ] ||
                    Object.entries(allProviders).find(([key]) =>
                      key.includes(k.key.toLowerCase().split("_")[0]),
                    )?.[1];
                    ...
                    >
                      <div>
                        <div className="flex items-center gap-2">
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${isConfigured ? "bg-green-400" : "bg-gray-600"}`}
                          />
                          <p className="text-xs text-gray-300">{k.label}</p>
                          {k.primary && (
                            <span className="text-[10px] text-blue-400 bg-blue-500/10 border border-blue-500/20 px-1 rounded">
                              Primary
                            </span>
                          )}
                        </div>
                        <p className="text-[10px] text-gray-600 ml-3.5 mt-0.5 font-mono">
                          {k.key}
                        </p>
                      </div>
              ...
              <p className="text-[10px] text-gray-600 mt-3">
                Set keys in <code className="text-blue-400">backend/.env</code>{" "}
          ...
          <div className="card p-5">
            ...
            <div className="space-y-2">
              {Object.entries(providers.projectdiscovery_tools || {}).map(
                ([name, info]) => (
                  <div
                    key={name}
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${info.available ? "bg-green-400" : "bg-red-400"}`}
                      />
                    ...
                    {info.available ? (
                      ...
                      <a
                        href="https://docs.projectdiscovery.io/opensource"
                        target="_blank"
                        rel="noreferrer"
            ...
            <p className="text-[10px] text-gray-600 mt-3">
              ...
              <code className="text-blue-400">scripts/install_pd_tools.sh</code>
            </p>
          </div>
        </>
      )}
