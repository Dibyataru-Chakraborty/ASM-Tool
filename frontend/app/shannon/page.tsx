...
const SEV_STYLE: Record<string, string> = {
  critical: 'tag-crit', high: 'tag-high', medium: 'tag-med', low: 'tag-low',
}
...
function FindingCard({ f }: { f: any }) {
  ...
  return (
    <div className={`card overflow-hidden border-l-2 ${
      s === 'critical' ? 'border-l-red-500' :
      s === 'high'     ? 'border-l-orange-500' :
      s === 'medium'   ? 'border-l-yellow-500' : 'border-l-blue-500'
    }`}>
      <button onClick={() => setOpen(!open)} className="w-full flex items-center gap-3 p-4 text-left hover:bg-[#1c2128] transition">
        <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${SEV_BAR[s] || 'bg-gray-400'}`} />
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            <SeverityTag s={f.severity} />
            <span className="text-xs text-gray-500">CVSS {f.cvss_score}</span>
            <span className="text-xs bg-[#0d1117] border border-[#30363d] text-gray-500 px-1.5 py-0.5 rounded">{f.vuln_class?.toUpperCase()}</span>
          </div>
          <p className="text-sm text-gray-200 font-medium truncate">{f.title}</p>
          <p className="text-xs text-gray-500 font-mono truncate">{f.target_url}</p>
        </div>
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3 border-t border-[#21262d]">
          <div className="pt-3 grid grid-cols-2 gap-3 text-xs">
            <div><p className="text-gray-500 mb-0.5">Parameter</p><p className="font-mono text-gray-300">{f.parameter || '—'}</p></div>
            <div><p className="text-gray-500 mb-0.5">Method</p><p className="font-mono text-gray-300">{f.method}</p></div>
          </div>

          {f.description && (
            <div><p className="text-xs text-gray-500 mb-1">Description</p>
            <p className="text-xs text-gray-300 leading-relaxed">{f.description}</p></div>
          )}
          ...
          {f.payload && (
            <div><p className="text-xs text-gray-500 mb-1">Payload</p>
            <code className="block bg-[#0d1117] border border-[#30363d] rounded-lg p-3 text-xs text-green-400 font-mono whitespace-pre-wrap break-all">{f.payload}</code></div>
          )}
          ...
          {f.curl_command && (
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <button onClick={() => navigator.clipboard.writeText(f.curl_command)}
                  className="text-[10px] text-blue-400 hover:text-blue-300">Copy</button>
              </div>
              <code className="block bg-[#0d1117] border border-[#30363d] rounded-lg p-3 text-xs text-cyan-300 font-mono whitespace-pre-wrap break-all">{f.curl_command}</code>
            </div>
          )}

          {f.poc && (
            <div><p className="text-xs text-gray-500 mb-1">Steps</p>
            <div className="text-xs text-gray-300 leading-relaxed whitespace-pre-line bg-[#0d1117] border border-[#30363d] rounded-lg p-3">{f.poc}</div></div>
          )}
...
function ShannonPageInner() {
  ...
  const [tab, setTab]       = useState<'findings' | 'surface' | 'report'>('findings')
  ...
  const start = async () => {
    ...
    try {
      const r = await axios.post(`${API}/api/v1/shannon/scan`, { target_url: url }, { headers: H() })
      setScanId(r.data.scan_id)
    } catch (e: any) {
      setError(e.response?.data?.detail || 'Failed — check API is running and GEMINI_API_KEY is set')
      setLoading(false)
    }
  }
  ...
  findings.forEach((f: any) => { counts[f.severity?.toLowerCase()] = (counts[f.severity?.toLowerCase()] || 0) + 1 })
      ...
      {!scan && !loading && (
        <div className="card p-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              { n:'1', t:'Crawl + Stack', d:'Discovers all endpoints, detects framework & auth' },
              { n:'2', t:'5× Gemini Agents', d:'Injection · XSS · Auth · AuthZ · SSRF — in parallel' },
              { n:'3', t:'Prove + Report', d:'Only confirmed exploits → Gemini writes full report' },
            ].map(s => (
              <div key={s.n} className="bg-[#0d1117] border border-[#21262d] rounded-lg p-3">
                <p className="text-blue-400 font-bold text-sm mb-1">Phase {s.n}</p>
                <p className="text-xs text-gray-300 font-medium mb-0.5">{s.t}</p>
            ...
            {PHASES.map((p, i) => {
              ...
              return (
                <div key={p.id} className="flex items-center gap-1 shrink-0">
                  <span className={`text-xs px-2 py-1 rounded font-medium transition ${
                    done   ? 'text-green-400 bg-green-500/10' :
                    active ? 'text-blue-300 bg-blue-500/10 animate-pulse' :
                             'text-gray-600 bg-[#0d1117]'
                  }`}>
                    {done ? '✓ ' : active ? '● ' : ''}{p.label}
                  </span>
                  {i < PHASES.length - 1 && <span className="text-gray-700 text-xs">›</span>}
                </div>
              )
            })}
          </div>
      ...
      {scan?.status === 'completed' && scan.report && (
        <div className="space-y-4">
          ...
          <div className="card p-4 flex items-center justify-between">
            ...
            <div className="text-xs text-gray-600 font-mono">#{scan.report.scan_id}</div>
          </div>

          {/* Stats */}
          <div className="grid grid-cols-4 gap-2">
            {[
              { label: 'Critical', count: counts.critical || 0, cls: 'text-red-400 bg-red-500/10 border-red-500/20' },
              { label: 'High',     count: counts.high     || 0, cls: 'text-orange-400 bg-orange-500/10 border-orange-500/20' },
              { label: 'Medium',   count: counts.medium   || 0, cls: 'text-yellow-400 bg-yellow-500/10 border-yellow-500/20' },
              { label: 'Low',      count: counts.low      || 0, cls: 'text-blue-400 bg-blue-500/10 border-blue-500/20' },
            ].map(s => (
              <div key={s.label} className={`rounded-xl border p-3 ${s.cls}`}>
                <p className={`text-2xl font-bold ${s.cls.split(' ')[0]}`}>{s.count}</p>
          ...
          {scan.report.summary && (
            <div className="card p-4">
              <p className="text-xs text-gray-500 font-semibold uppercase tracking-wider mb-2">Executive Summary</p>
              <p className="text-sm text-gray-300 leading-relaxed">{scan.report.summary}</p>
            </div>
          )}
            ...
            <div className="card p-4 space-y-4">
              <div className="grid grid-cols-2 gap-3 text-sm">
                {[
                  ...
                  <div key={l}><p className="text-xs text-gray-500 mb-0.5">{l}</p><p className="text-gray-200 font-mono text-xs">{v || '—'}</p></div>
                ))}
              </div>
              {scan.report.attack_surface.technologies?.length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {scan.report.attack_surface.technologies.map((t: string) => (
              ...
              {scan.report.attack_surface.endpoints?.length > 0 && (
                <div>
                  ...
                  <div className="space-y-1">
                    {scan.report.attack_surface.endpoints.slice(0, 8).map((e: any, i: number) => (
                      <div key={i} className="flex gap-2 text-xs bg-[#0d1117] border border-[#21262d] rounded-lg px-3 py-1.5">
                        <span className="text-blue-400 font-mono shrink-0">{e.method}</span>
          ...
          {tab === 'report' && (
            <div className="card overflow-hidden">
              <div className="flex items-center justify-between px-4 py-2.5 border-b border-[#21262d]">
                <span className="text-xs text-gray-500">Full Report — Written by Gemini 1.5 Pro</span>
                <button onClick={() => {
                  const blob = new Blob([scan.report.markdown], { type: 'text/markdown' })
...
export default function ShannonPage(props: any) {
  return (
    <AuthProvider>
      <AppLayout>
