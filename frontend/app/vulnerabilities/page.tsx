...
export default function VulnsPage() {
  const [vulns, setVulns] = useState<Finding[]>([])
  const [scans, setScans] = useState<FindingScan[]>([])
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-xs text-gray-500">
            {total} real scanner findings and observations from saved scan history
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-5">
          {['critical', 'high', 'medium', 'low', 'info'].map(severity => (
            ...
            >
              <p className={`text-xl font-bold ${SEV_CLS[severity]?.split(' ')[0] || 'text-gray-400'}`}>
                {counts[severity] || 0}
              </p>
          ...
          >
            ...
            {scans.map(scan => (
              <option key={scan.id} value={scan.id}>
                {scan.reference_id} · {scan.target_domain || 'Unknown target'} ({scan.finding_count})
              </option>
            ))}
          </select>
          <button type="button" onClick={clearFilters} className="btn-gray text-xs">
            Clear
          </button>
        </div>
        ...
        {error && (
          <div className="flex items-center justify-between rounded-lg border border-red-500/20 bg-red-500/5 px-4 py-3">
            ...
            <button type="button" onClick={load} className="text-xs text-blue-400 hover:text-blue-300">
              Retry
            </button>
          </div>
        )}
        ...
        <div className="card overflow-x-auto">
          {loading ? (
            <div className="py-12 text-center text-sm text-gray-600 animate-pulse">Loading…</div>
          ) : vulns.length === 0 ? (
            <div className="py-12 text-center">
              <p className="mb-2 text-2xl">🐛</p>
              <p className="text-sm text-gray-500">No vulnerabilities found</p>
              <p className="mt-1 text-xs text-gray-600">
                {sev || scanId ? 'Clear the filters to view all saved findings' : 'Run a scan to discover real vulnerabilities'}
              </p>
            </div>
          ) : (
            <table className="w-full min-w-[1180px] text-xs">
              <thead>
                <tr className="border-b border-[#21262d]">
                  {['Title', 'Severity', 'CVSS', 'CVE', 'Host', 'Scan ID', 'Date', 'Actions'].map(header => (
                ...
                {vulns.map(vulnerability => (
                  ...
                  >
                    <td className="max-w-xs px-4 py-2.5">
                      <Link href={`/vulnerabilities/${vulnerability.id}`} className="block truncate text-gray-200 hover:text-blue-400">
                        {vulnerability.title}
                      </Link>
                      <div className="mt-0.5 flex gap-2 text-[10px] text-gray-600">
                        {vulnerability.source && <span>{vulnerability.source}</span>}
                        {vulnerability.is_false_positive && <span>False positive</span>}
                      </div>
                    </td>
                    <td className="px-4 py-2.5">
                      <span className={SEV_CLS[vulnerability.severity] || 'tag-info'}>
                    ...
                    <td className="px-4 py-2.5">
                      {vulnerability.cvss_score == null ? (
                        ...
                        <span className={`text-xs font-bold ${vulnerability.cvss_score >= 9 ? 'text-red-400' : vulnerability.cvss_score >= 7 ? 'text-orange-400' : vulnerability.cvss_score >= 4 ? 'text-yellow-400' : 'text-blue-400'}`}>
                    ...
                    <td className="max-w-[160px] px-4 py-2.5 font-mono text-gray-400">
                      <p className="truncate" title={vulnerability.host || undefined}>{vulnerability.host || '—'}</p>
                      {vulnerability.port != null && <p className="text-[10px] text-gray-600">Port {vulnerability.port}</p>}
                    </td>
                    <td className="px-4 py-2.5">
                      <Link
                      ...
                      >
                        {vulnerability.scan_reference || vulnerability.scan_id}
                      </Link>
                    ...
                    <td className="px-4 py-2.5 text-gray-500">
                      {vulnerability.created_at ? new Date(vulnerability.created_at).toLocaleDateString() : ''}
                    </td>
                    <td className="px-4 py-2.5">
                      <div className="flex gap-2">
                        <Link href={`/vulnerabilities/${vulnerability.id}`} className="text-xs text-blue-400 hover:text-blue-300">
                          Detail
                        </Link>
                        ...
                        >
                          {vulnerability.is_false_positive ? 'Restore' : 'FP'}
                        </button>
                      </div>
                    </td>
                  </tr>
