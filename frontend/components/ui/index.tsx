...
export function SeverityBadge({ severity }: { severity: Severity | string }) {
  const normalized = typeof severity === 'string' && severity
...
export function StatusBadge({ status }: { status: string }) {
  const map: Record<string, string> = {
    active: 'badge-low',
    running: 'bg-blue-500/10 text-blue-400 border border-blue-500/20 px-2 py-0.5 rounded text-xs font-medium',
    completed: 'bg-green-500/10 text-green-400 border border-green-500/20 px-2 py-0.5 rounded text-xs font-medium',
    failed: 'badge-critical',
    pending: 'badge-medium',
    cancelled: 'badge-info',
    archived: 'badge-info',
    open: 'badge-critical',
    resolved: 'bg-green-500/10 text-green-400 border border-green-500/20 px-2 py-0.5 rounded text-xs font-medium',
  }
  return (
